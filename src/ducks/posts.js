      ...
      return {
        ...
        userPosts: state.userPosts.map(post => {
          if (post.noteId === action.payload.noteId) {
            return {
              ...post,
              tags: post.tags.filter(tag => tag.tagId !== action.payload.tagId)
            };
          } else {
            return post;
          }
    ...
    case actionTypes.REPLACE_POST_WITH_TAGGED_POST:
      return {
        ...
        userPosts: state.userPosts.map(note => {
          return note.noteId === action.payload.noteId ? action.payload : note;
        })
      };
    ...
    fetch(`${baseUrl}/tags/`, {
      ...
      .then(response => {
        ...
        if (response.status !== 200) {
          dispatch(alertError("Tag could not be deleted at this time"));
        }
      })
      .catch(err => {
        console.log(err);
  ...
  return dispatch => {
    ...
    fetch(`${baseUrl}/notes/${noteId}`, {
      ...
      .then(response => {
        console.log(response);
        if (response.status !== 200) {
          dispatch(alertError("Note could not be deleted at this time"));
        }
      })
      .catch(err => {
        console.log(err);
